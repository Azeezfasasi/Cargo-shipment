...
export default function Hero() {
  ...
  return (
    <section className="relative bg-red-600 font-sans overflow-hidden">
      <div className="container mx-auto flex flex-col lg:flex-row items-center justify-between py-12 lg:py-20 px-4">
        {/* Left Section: Text Content */}
        <div className="lg:w-1/2 w-full bg-red-600 text-white p-8 lg:p-16 rounded-lg shadow-lg flex flex-col justify-center relative z-10">
          ...
          <Link href={slides[current].cta} className="flex items-center justify-center px-8 py-4 bg-white text-red-600 font-bold rounded-full shadow-md hover:bg-gray-100 transition duration-300 ease-in-out self-start">
            Get a Quote
            <svg
        ...
        <div className="hidden md:block lg:w-1/2 w-full mt-8 lg:mt-0 lg:ml-12 relative">
          ...
          />
          <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-transparent to-red-600 opacity-10 rounded-lg pointer-events-none"></div>
          {/* Navigation Arrows */}
          <button
          ...
          <button
            className="absolute right-2 top-1/2 transform -translate-y-1/2 bg-white text-red-600 rounded-full p-2 shadow hover:bg-gray-100 z-20"
            onClick={goToNext}
      ...
      <div className="absolute bottom-6 left-0 right-0 flex justify-center items-center space-x-2 z-30">
        {slides.map((_, idx) => (
          <button
            key={idx}
            className={`w-3 h-3 rounded-full transition-all duration-300 ${current === idx ? 'bg-red-600 scale-125' : 'bg-gray-300'}`}
